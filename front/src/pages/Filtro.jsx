...
export default function Filtro() {
  ...
  const aplicarFiltros = (e) => {
    ...
    if (filtros.precoMaximo && Number(filtros.precoMaximo) < 0) {
      alert("Preço máximo não pode ser negativo.");
      return;
    }
    if (
      filtros.avaliacaoMinima &&
      (Number(filtros.avaliacaoMinima) < 0 || Number(filtros.avaliacaoMinima) > 5)
    ) {
      alert("Avaliação mínima deve estar entre 0 e 5.");
      return;
    }

    // navega passando state (seu Resultados.jsx lê via useLocation)
